...
//		$("").html('Some text with &lt;div&gt;html&lt;/div&gt;').text()	
		...
		cleanAnswer = jQuery('<span>').html(answer).text().toLowerCase();		
		cleanAttempt = jQuery('<span>').html(attempt).text().toLowerCase();
		if (cleanAnswer != cleanAttempt)
		{
			cleanAnswer = accentFold(cleanAnswer.trim());
			cleanAttempt = accentFold(cleanAttempt.trim());
		}
